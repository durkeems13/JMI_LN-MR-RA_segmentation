...
def work(filepaths,savefolder,fldr,d_annot_index,image_id):
    ...
    for filepath in filepaths:
        ...
        d_stack=np.stack([image[1],image[2],image[5]])
        #Processes and splits overlays, generates annotations
        points=process_overlays(overlays)

        new_d = TiffWriter(os.path.join(savefolder,'dcs',fldr,filename))
...
def main():
    ...
    if not os.path.exists(os.path.join(savefldr,'dcs')):
        os.makedirs(os.path.join(savefldr,'dcs'))
    for fldr in ['train','val','testset','annotations']:
        if not os.path.exists(os.path.join(savefldr,'dcs',fldr)):
            os.makedirs(os.path.join(savefldr,'dcs',fldr))
    #Drop ims with no DCs 
    collection=list(glob(os.path.join(read_dir,'*.tif')))
    collection=eliminate_empty(collection)
    print(len(collection))
    #Split dataset into train/test/val
    ##DUMMY CODE FOR GITHUB##
    #load pkl with patient IDs after splitting at patient level
    train_per=0.8
    val_per=0.1
    shuffle(collection)
    mid=int(len(collection)*train_per)
    mid2=int(len(collection)*(train_per+val_per))
    train_rois=collection[:mid]
    val_rois=collection[mid:mid2]
    test_rois=collection[mid2:]
    ##DUMMY CODE FOR GITHUB DONE##
    means=[]
    for x in train_rois:
        im=imread(x)
    ...
    cats_d=[{'name':'bdca2','id':1},{'name':'cd11c','id':2}]
    

    train_json_dict_d={'info':{'description':'my new train dataset DCs','means':list(b)},
        'images':train_ims,'annotations':tr_d_annotations,'categories':cats_d}
    with open(os.path.join(savefldr,'dcs','annotations','train.json'),'w') as outfile:
        json.dump(train_json_dict_d,outfile)

    val_json_dict_d={'info':{'description':'my new val dataset dcs'},
        'images':val_ims,'annotations':v_d_annotations,'categories':cats_d}
    with open(os.path.join(savefldr,'dcs','annotations','val.json'),'w') as outfile:
    ...
    with open(os.path.join(savefldr,'dcs','annotations','testset.json'),'w') as outfile:
