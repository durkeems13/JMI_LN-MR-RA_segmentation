...
import sys
# append directory above this one to system path
# so code can be found
...
class ResNetC4Model(DetectionModel):
    def inputs(self):
        ret = [
            tf.placeholder(tf.float32, (None, None, 3), 'image'), #changed 6 to 3
...
def predict(pred_func,input_dir,input_file,output_dir ):
    #img = cv2.imread(input_file, cv2.IMREAD_COLOR)
    img = imread(os.path.join(input_dir,input_file))
    print('***',img.shape,input_file)
    img = np.stack([img[i,...] for i in range(img.shape[0])],axis=2)
    img=img.astype(np.float32)
    img=img/np.power(2.0,16)
    img=img.astype(np.float32)
    ...
    #print(viz.shape)
    #tpviz.interactive_imshow(viz)
    pkl.dump(results,open(os.path.join(output_dir,input_file[:-4]+'.pkl'),'wb'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--index', help='load a model for evaluation or training. Can overwrite BACKBONE.WEIGHTS')
    ...
    parser.add_argument('--output_dir', help="Run prediction on a given image. "
                                          "This argument is the path to the input image file")
    parser.add_argument('--tasks', help="Run prediction on a given image. "
                                          "This argument is the path to the input image file")
    ...
    if args.visualize or args.evaluate or args.predict:
        ...
        if args.visualize:
            ...
            if args.evaluate:
                ...
                COCODetection('../../data/LN_DC_trainset','val')   # Only to load the class names into caches
                index=int(args.index)
                tasks=int(args.tasks)
                from glob import glob
                #fs=list(os.listdir(args.predict))
                fs=list(glob(os.path.join(args.predict,'*.tif')))
                fs=[x.split('/')[-1] for x in fs]
               
                fs=fs[int(index*(len(fs)/tasks)):int((index+1)*(len(fs)/tasks))]
                for x in fs:
                    predict(pred, args.predict, x, args.output_dir)
    else:
